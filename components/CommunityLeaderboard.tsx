...
import { fetchLeaderboard, type LeaderboardEntry, type LeaderboardResponse } from '@/utils/leaderboardApi';
...
export function CommunityLeaderboard({ isDark, refreshKey }: CommunityLeaderboardProps) {
  ...
  const loadTab = useCallback(async (tab: Tab) => {
    console.log('[CommunityLeaderboard] Loading tab:', tab);
    if (tab === 'steps') {
      setLoadingSteps(true);
      try {
        const data = await fetchLeaderboard('steps', 'today');
        setStepsData(data);
      } catch (err) {
      ...
      setLoadingGym(true);
      try {
        const data = await fetchLeaderboard('gym', 'month');
        setGymData(data);
      } catch (err) {
  ...
  // Determine top 5 and user-outside-top-5
  ...
  return (
    <View style={[styles.container, { backgroundColor: cardBg, borderColor: cardBorder }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.subtitle, { color: subColor }]}>This week</Text>
      </View>
