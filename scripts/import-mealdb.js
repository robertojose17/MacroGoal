...
async function getOFFMacros(ingredientName) {
  ...
  for (const [knownKey, macros] of Object.entries(KNOWN_MACROS)) {
    if (key === knownKey || key.includes(knownKey) || knownKey.includes(key)) {
      console.log(`   [KNOWN] "${key}" → ${macros.calories} kcal`);
  ...
  try {
    ...
    console.log(`   [OFF] Best match for "${key}": "${best.name}" (score ${best.score}) → ${best.macros.calories} kcal, ${best.macros.protein}g P, ${best.macros.carbs}g C, ${best.macros.fat}g F`);
