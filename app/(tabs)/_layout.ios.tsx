...
export default function TabLayout() {
  ...
  useEffect(() => {
    ...
    const checkReferralPrompt = async () => {
      try {
        ...
        if (profileError) {
          console.warn('[Tab Layout iOS] Supabase profile query failed, showing modal anyway:', profileError);
        } else if (profile?.referral_prompt_shown) {
          await AsyncStorage.setItem(REFERRAL_PROMPT_KEY, 'true');
          return;
        }

        console.log('[Tab Layout iOS] First launch — showing referral code prompt');
        ...
        console.warn('[Tab Layout iOS] Failed to check referral prompt:', e);
      }
    };
  ...
  const dismissReferralModal = async () => {
    console.log('[Tab Layout iOS] Referral modal dismissed');
    ...
    try {
      ...
      console.warn('[Tab Layout iOS] Failed to save referral prompt flag:', e);
    }
  };

  const handleApplyReferralCode = async () => {
    if (!referralCode.trim()) return;
    console.log('[Tab Layout iOS] Apply referral code pressed:', referralCode.trim());
    setReferralApplying(true);
    try {
      const result = await applyReferralCode(referralCode.trim());
      if (result.success) {
        console.log('[Tab Layout iOS] Referral code applied successfully');
        setReferralApplying(false);
        Alert.alert('🎉 Code Applied!', 'You and your friend both earned 1,000 XP!');
        await dismissReferralModal();
      } else {
        console.warn('[Tab Layout iOS] Referral code failed:', result.error);
        setReferralApplying(false);
        Alert.alert('Invalid Code', result.error || 'Could not apply this code.');
      }
    } catch (e) {
      console.error('[Tab Layout iOS] Unexpected error applying referral code:', e);
      setReferralApplying(false);
